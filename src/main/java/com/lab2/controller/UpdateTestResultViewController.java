...
public class UpdateTestResultViewController {

	public TestResult getNewlyUpdatedTestResult() {
    ...
    @FXML
    private void handleUpdateTestResult() {
		try {
			...
			newTestResult = new TestResult(newTestResult.getId(), visit.getId(), test.getId(), testResult);

			testResultDao.update(newTestResult);
			closeStage();
		} catch (Exception e) {
